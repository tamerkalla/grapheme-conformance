...
describe(".github/workflows/ci.yml", () => {
  ...
  test("the test job runs install, typecheck, test, build and smoke, in order", () => {
  ...
  test("the scoreboard job checks reproducibility", () => {
    const steps = ci.jobs.scoreboard.steps;
    const runSteps = steps.filter((s) => typeof s.run === "string").map((s) => s.run);
    expect(runSteps).toEqual(["npm ci", "npm run scoreboard:check"]);
  });
});
...
describe(".github/workflows/release.yml", () => {
  ...
  test("the bump step's version output is read from package.json, never captured from npm version's stdout", () => {
    // npm version prints the new version WITH a leading "v" ("v0.1.4"). The
    ...
    // read of package.json, the tag and release title come out "vv0.1.4".
